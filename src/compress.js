...
export default  function useCompressedLocalStorageState(key, defaultValue) {
        ...
        if (compressedStore == "") {
            ...
            compressWasm(String(defaultValue)).then((compressed) => {
                ...
                deCompressWasm(compressedStore).then((decompressed) => {
                    console.log("DECOMPRE", compressedStore, decompressed,
                        compressedStore.length, decompressed.length)
    ...
    function setup() {
        ...
        var path = "https://unpkg.com/wasm-flate@0.1.12-alpha/dist/9eae6f5a6ee0cd2a3640.module.wasm"
    ...
    function compressWasm(stringToCompress) {
        return WebAssembly.instantiate(obj)
            .then(function(skee) {
        ...
        () => {
            ...
            if (obj != null) {
                ...
                compressWasm(toCompress).then((compressed) => {
                    console.log("COMPRESS", toCompress, compressed,
                        toCompress.length, compressed.length)
                    window.localStorage.setItem(key + "compressed", compressed)
                })
